...
- Linear: confirm the authenticated user is `alexander.d.mills@gmail.com`. Require the user to identify the intended Ores workspace by name or ID when multiple workspaces are available, then resolve the team, project, and status rather than guessing them.
